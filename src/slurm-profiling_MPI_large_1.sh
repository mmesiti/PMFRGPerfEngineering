...
SCRIPT="$PROJECT/src/slurm-profiling_MPI_large_1.sh"
...
profiledata = "../profile-toy-$slurm_job_id.txt"
println("Save profling data in $profiledata")
open(profiledata, "w") do s
    Profile.print(IOContext(s, :displaysize => (24, 1000)))
end
Profile.clear()
...
Profile.init(n=5*10^8,delay=0.001)
@profile @time Solution, saved_values = SolveFRG(
    Par,
    UseMPI(),
    MainFile=mainFile,
    CheckpointDirectory=flowpath,
...
profiledata = "../profile-real-$slurm_job_id.txt"
println("Save profling data in $profiledata")
open(profiledata, "w") do s
